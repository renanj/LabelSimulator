...
from _05_01_building_blocks import f_cold_start, bblocks
...
def f_framework_df(
    ...
    _array_unlabels_sample_ids = cp.array(df_train['sample_id'][~df_train['sample_id'].isin(_cold_start_samples_id)])    
    ...
    while len(_array_unlabels_sample_ids) > 0:  
        ...
        if _array_ordered_samples_id is not None:		
            ...
            selected_sample_id = _array_ordered_samples_id[_temp_batch_size:_query_batch_size]
        ...
        _total_samples_evaluated = len(selected_sample_id)
        _total_samples_evaluated_string = str(_total_samples_evaluated) + '/' + str(_total_samples) 
    ...
    _pd_list_6 = _array_total_samples_evaluated.tolist() #10/100, 20/100,... 100/100
    _pd_list_7 = _array_samples_ids_per_batch.tolist() #[1,5,3,2,...], [...], [...]
    _pd_list_8 = _array_score_train.tolist() 
    _pd_list_9 = _array_score_validation.tolist() 

    _result_df = pd.DataFrame(list(zip(_pd_list_0, _pd_list_1, _pd_list_2, _pd_list_3, _pd_list_4, _pd_list_5 , _pd_list_6, _pd_list_7,_pd_list_8, _pd_list_9))
...
for db_paths in _list_data_sets_path:
    ...
    for _deep_learning_arq_sub_folder_name in _deep_learning_arq_sub_folders:									
        ...
        _df_train = pd.read_pickle('df_train.pkl')
        _df_faiss_indices = pd.read_pickle('df_faiss_indices_train.pkl')
        _df_faiss_distances = pd.read_pickle('df_faiss_distances_train.pkl')
        _df_validation = pd.read_pickle('df_validation.pkl')
        
        _df_2D_train = pd.read_pickle('df_2D_train.pkl')
        _df_2D_faiss_indices = pd.read_pickle('df_2D_faiss_indices_train.pkl')
        _df_2D_faiss_distances = pd.read_pickle('df_2D_faiss_distances_train.pkl')
        _df_2D_validation = pd.read_pickle('df_2D_validation.pkl')


        _random_samples_id, _cold_start_samples_id = f_cold_start(_df_train)
